			...
			$app->enqueueMessage('<h2>' . JText::sprintf('COM_ICAGENDA_PRO_WELCOME', 'iCagenda PRO') . '</h2>'
						...
						<div>
							<h2 style="font-size:2em;">
								<b style="color:#cc0000;">iC</b><b style="color: #666666;">agenda<sup style="font-size:0.5em">&trade;</sup></b><small><?php echo $version;?></small>
							</h2>
						</div>
						<div>
							<h4>
...
<?php foreach ($charts as $chart => $title) : ?>
	...
	foreach ($this->{$chart} AS $v)
	{
		...
		$script[]= '			label: "' . $stats_label . '"';
		$script[]= '		},';

		$legend_data = $v->cat_title . '@@' . $v->cat_color;
