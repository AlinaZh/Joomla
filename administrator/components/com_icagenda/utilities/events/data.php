...
class icagendaEventsData
{
	 ...
	 * @since 3.5.0
	 */
	public static function getAllDates($filterTime = null, $datesDisplay = null, $orderby = null, $mcatid = null, $isModule = false)
	{
...
//		$query->select('e.next, e.dates, e.startdate, e.enddate, e.period, e.weekdays, e.displaytime, e.id, e.catid');
		...
		if ($mcatid != 'all')
		{
			$filterCategory = $app->getUserStateFromRequest('filter.category', 'filter_category', $mcatid);
		...
		else
		{
			...
			$filterFrom		= $app->getUserStateFromRequest('filter.from', 'filter_from');
			$filterTo		= $app->getUserStateFromRequest('filter.to', 'filter_to');
			$filterMonth	= $app->getUserStateFromRequest('filter.month', 'filter_month');
			$filterYear		= $app->getUserStateFromRequest('filter.year', 'filter_year');
		}
		...
		foreach ($list AS $i)
		{
			...
			foreach ($singledates as $sd)
			{
				...
				if ($isValid)
				{
						...
						)
					{
						...
						{
							$dateToDisplay = (strtotime($date_Dat) >= strtotime($filterFrom)) ? true : false;
						}

						// Filter by period - To
						if ( ! empty($filterTo) && $dateToDisplay)
						{
							$dateToDisplay = (strtotime($date_Dat) <= strtotime($filterTo)) ? true : false;
						}

						// Filter by Month
						if ( ! empty($filterMonth) && $dateToDisplay)
						{
							$dateToDisplay = (date('n', strtotime($date_Dat)) == $filterMonth) ? true : false;
						}

						// Filter by Year
						if ( ! empty($filterYear) && $dateToDisplay)
						{
							$dateToDisplay = (date('Y', strtotime($date_Dat)) == $filterYear) ? true : false;
						}
				...
				&& count($AllDatesDisplay) > 0 )
			{
				...
				if ($filterTime != '4')
				{
					...
					elseif ( $min_date >= $datetime_today )
					{
			...
			foreach ($AllDatesDisplay as $fD)
			{
					...
					)
				{
					...
					if ( ! empty($filterFrom) && $dateToDisplay)
					{
						$dateToDisplay = (strtotime($date_get_date) >= strtotime($filterFrom)) ? true : false;
					}

					// Filter by period - To
					if ( ! empty($filterTo) && $dateToDisplay)
					{
						$dateToDisplay = (strtotime($date_get_date) <= strtotime($filterTo)) ? true : false;
					}

					// Filter by Month
					if ( ! empty($filterMonth) && $dateToDisplay)
					{
						$dateToDisplay = (date('n', strtotime($date_get_date)) == $filterMonth) ? true : false;
					}

					// Filter by Year
					if ( ! empty($filterYear) && $dateToDisplay)
					{
						$dateToDisplay = (date('Y', strtotime($date_get_date)) == $filterYear) ? true : false;
					}
